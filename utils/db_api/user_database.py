...
def init_user_db():
    ...
    conn.commit()
    conn.close()
    print("✅ stats.db yaratildi - 5 ta bepul urinish bilan!")
...
def create_user(telegram_id, full_name, username=None, phone_number=None):
    ...
    try:
        ...
        if user:
            ...
            # ✅ YANGI USER - 5 TA BEPUL URINISH bilan yaratish
            c.execute("""
                INSERT INTO users 
...
def update_phone_number(telegram_id, phone_number):
    ...
    try:
        ...
        return {
            'success': True,
            'message': '✅ Telefon raqami saqlandi'
        }
    except Exception as e:
...
def check_can_price(telegram_id):
    """
    ✅ NARXLASH MUMKINLIGINI TEKSHIRISH (TO'G'RILANGAN)
    ...
    try:
        ...
        if not phone_number:
            return {
                ...
                'message': '📱 Iltimos, avval telefon raqamingizni kiriting'
            }

        # 2️⃣ LOCAL bepul urinishlar
        if free_trials > 0:
            return {
                'success': True,
                'can_price': True,
                'is_free': True,
                'free_trials_left': free_trials,
                'message': f'🎁 Bepul urinish: {free_trials} ta qoldi'
            }

        # 3️⃣ free tugagan -> bu yerda "can_price" false qaytaramiz
...
def use_pricing(telegram_id, phone_model, storage='', color='', battery='', sim_type='', has_box='',
                damage='', price=0):
    """
    ✅ LOCAL use_pricing (TO'G'RILANGAN)

    Faqat BEPUL urinish bo'lsa ishlaydi.
    Paid balance'ga tegmaydi (paid API'da).
    """
    conn = get_user_conn()
    c = conn.cursor()
    ...
    try:
        ...
        # faqat free bo'lsa ruxsat
        if not check.get('can_price') or not check.get('is_free'):
            return {
                'success': False,
                'error': '❌ Bepul urinish qolmagan (paid balans API orqali yechiladi)'
            }

        # User ID olish
        c.execute("SELECT id, free_trials_left FROM users WHERE telegram_id = ?", (telegram_id,))
        user = c.fetchone()
        if not user:
        ...
        if free_trials <= 0:
            return {'success': False, 'error': 'Bepul urinish qolmagan'}

        # ✅ BEPUL URINISHDAN FOYDALANISH (minus 1)
        c.execute("""
            UPDATE users
        ...
        c.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
        """, (user_id, telegram_id, phone_model, storage, color, battery, sim_type, has_box, damage, price))
        ...
        return {
            ...
            # paid balans localda yo'q, shuning uchun None yoki 0
            'balance': None,
            'message': '✅ Bepul urinish ishlatildi!'
        }
...
def admin_add_balance(telegram_id, count):
    ...
    try:
        ...
        print(f"❌ Admin balans qo'shishda xato: {e}")
        return {
            'success': False,
            'error': str(e)
        }
...
def admin_reset_free_trials(telegram_id, count=5):
    ...
    try:
        ...
        return {
            'success': True,
            'message': f'🎁 Bepul urinishlar qayta tiklandi: {count} ta'
        }
    except Exception as e:
        print(f"❌ Bepul urinishlarni tiklashda xato: {e}")
        return {
            'success': False,
            'error': str(e)
        }
...
def get_registered_users_count():
    ...
    try:
        ...
        print(f"❌ Ro'yxatdan o'tganlar sonini olishda xato: {e}")
        return 0
    finally:
        conn.close()
...
def get_active_users(days=7):
    ...
    try:
        ...
        print(f"❌ Faol foydalanuvchilarni olishda xato: {e}")
        return {
            'success': False,
            'error': str(e),
            'total': 0,
...
def get_global_stats():
    ...
    try:
        ...
        print(f"❌ Global statistikani olishda xato: {e}")
        return {
            'success': False,
            'error': str(e),
            'stats': {},
...
def get_total_price_inquiries(period='all'):
    ...
    try:
        ...
        print(f"❌ Narxlash so'rovlarini olishda xato: {e}")
        return {
            'success': False,
            'error': str(e),
            'total': 0,
...
def get_user_growth_stats(days=30):
    ...
    try:
        ...
        print(f"❌ Foydalanuvchilar o'sish statistikasini olishda xato: {e}")
        return {
            'success': False,
            'error': str(e),
            'daily_growth': [],
...
if __name__ == "__main__":
    init_user_db()
    print("✅ User database tayyor - 5 ta bepul urinish bilan!")