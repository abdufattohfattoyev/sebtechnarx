...
async def start(message: types.Message, state: FSMContext):
    ...
    logger.info(f"👤 User started bot: {user.id} - {user.full_name}")

    # ✅ 1. USER YARATISH (BACKEND API ORQALI)
    try:
        ...
        # ✅ BACKEND'DAN PHONE VA BALANS OLISH
        phone_number = result.get('phone')  # ✅ TO'G'RI: 'phone' (phone_number emas!)
        balance = result.get('balance', 0)

        logger.info(f"✅ User created/updated: {user.id}, phone: {phone_number}, balance: {balance}")

    except Exception as e:
        logger.error(f"❌ API error on start: {e}", exc_info=True)
        await message.answer(
            "❌ Serverga ulanishda xatolik. Iltimos, keyinroq /start bosing.",
            parse_mode="HTML"
        )
        return

    # ✅ 2. LOCAL DB'DA HAM USER YARATISH/YANGILASH (5 ta bepul urinish bilan)
    try:
        ...
        if local_result.get('success'):
            ...
            if local_result.get('is_new'):
                logger.info(f"🎁 Yangi user local DB'da yaratildi: {user.id}, free_trials: {free_trials}")
            else:
                logger.info(f"♻️ User local DB'da yangilandi: {user.id}, free_trials: {free_trials}")
        else:
            logger.warning(f"⚠️ Local DB user creation failed: {local_result.get('error')}")
            free_trials = 5

    except Exception as e:
        logger.warning(f"⚠️ Local DB error: {e}")
        free_trials = 5
...
async def receive_phone(message: types.Message, state: FSMContext):
    ...
    # ✅ 2. BACKEND API'GA TELEFON YUBORISH
    try:
        result = await api.update_phone(message.from_user.id, phone_number)

        if not result.get('success'):
            logger.error(f"❌ Backend phone update failed: {result.get('error')}")
            await message.answer(
                "❌ Telefon raqamini saqlashda xatolik. Qaytadan /start bosing.",
                parse_mode="HTML"
            )
            await state.finish()
            return

        logger.info(f"✅ Phone updated in backend: {message.from_user.id} -> {phone_number}")

    except Exception as e:
        logger.error(f"❌ Backend phone update error: {e}")
        await message.answer(
            "❌ Serverga ulanishda xatolik. Qaytadan /start bosing.",
            parse_mode="HTML"
        )
        await state.finish()
        return

    # ✅ 3. LOCAL DB'GA HAM SAQLASH (sinxronizatsiya uchun)
    try:
        from utils.db_api.user_database import update_phone_number
        update_phone_number(message.from_user.id, phone_number)
        logger.info(f"✅ Phone also saved in local DB: {message.from_user.id}")
    except Exception as e:
        logger.warning(f"⚠️ Local DB phone update error: {e}")
        # Backend'da saqlanganligini bildik, davom ettiramiz

    # ✅ 4. BALANS OLISH (Backend API'dan)
    try:
    ...
    # ✅ 5. BEPUL URINISHLAR (Local DB'dan)
    try:
    ...
    # ✅ 6. MODELLAR SONINI OLISH
    try:
        from utils.db_api.database import get_models
        models = get_models()
        models_text = f"✅ <b>{len(models)}</b> ta model mavjud\n"
    except Exception as e:
        logger.warning(f"⚠️ Models fetch error: {e}")
        models_text = ""

    # ✅ 7. MUVAFFAQIYATLI XABAR
    text = f"""✅ <b>Telefon raqami qabul qilindi!</b>

📞 <b>Raqam:</b> {phone_number}
...
"""
    ...
    if balance > 0:
        text += f"✅ Siz {balance} marta telefon narxlashingiz mumkin."
    elif free_trials > 0:
        text += f"🎁 Sizda {free_trials} ta bepul urinish mavjud!"
    else:
        text += (
            "⚠️ Balans yetarli emas!\n"
            "💳 Hisobni to'ldirish uchun <b>'💰 Hisobni to'ldirish'</b> tugmasini bosing."
        )
...
async def start_payment(message: types.Message, state: FSMContext):
    ...
    for tariff in tariffs:
        ...
        emoji = '1️⃣' if tariff['count'] == 1 else '5️⃣' if tariff['count'] == 5 else '🔟'
...
3️⃣ To'lov qilganingizdan so'ng "✅ To'lov qildim" tugmasini bosing
...
async def choose_model(message: types.Message, state: FSMContext):
    ...
    if not local_check.get("success"):
        await message.answer("❌ Local bazada user topilmadi. /start bosing.")
        return

    # Telefon raqami kerak bo'lsa
    if local_check.get("need_phone"):
        await message.answer(local_check.get("message", "📱 Iltimos, telefon raqam kiriting"))
        return
    ...
    if free_trials <= 0:
        try:
            ...
            logger.error(f"❌ API balansini olishda xatolik: {e}")
            api_balance = 0
...
🎁 <b>Bepul urinishlar:</b> 0 ta
💰 <b>Sotib olingan balans:</b> {api_balance} ta

💡 <b>Hisobni to'ldirish uchun quyidagi tugmani bosing:</b>
"""
    ...
    # 5) Status matni
    if free_trials > 0:
        status_text = f"🎁 <b>Bepul urinish: {free_trials} ta qoldi</b>"
    else:
        status_text = f"💰 <b>Balans: {api_balance} ta narxlash</b>"
...
async def model_selected(message: types.Message, state: FSMContext):
    ...
    if not model:
        await message.answer("❌ Model topilmadi. Iltimos, tugmalardan birini tanlang.")
        return
    ...
    if not storages:
        await message.answer("❌ Xotiralar topilmadi")
        return
...
async def parts_choice_selected(message: types.Message, state: FSMContext):
    ...
    if message.text == "✅ Ha":
        ...
        await message.answer("⬇️ Quyidagi qismlardan tanlang:", reply_markup=markup)

        await UserState.waiting_parts.set()
        return

    await message.answer("❌ Iltimos, tugmalardan birini tanlang:", reply_markup=parts_choice_kb())
...
async def parts_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data == "part_done":
        ...
        # ✅ TO'G'RI: Callback uchun maxsus funksiya
...
async def show_final_price_from_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    # ✅ 2) Agar free trial bo'lsa -> LOCAL yechamiz
    ...
    # ✅ 3) Free tugagan -> API paid balansdan yechamiz
    else:
        try:
            ...
            await call.message.answer(
                "❌ Server bilan bog'lanishda xatolik (API). Keyinroq urinib ko'ring.",
                reply_markup=main_menu(user_id in ADMINS)
            )
            await state.finish()
            return
        ...
        if not api_res.get("success"):
            await call.message.answer(
                f"❌ {api_res.get('error', 'Balans yetarli emas')}",
                reply_markup=main_menu(user_id in ADMINS)
            )
            await state.finish()
            return
    ...
    text = f"""📊 <b>HISOBLASH NATIJASI</b>

📱 <b>Model:</b> {data['model_name']}
💾 <b>Xotira:</b> {data['storage']}
🎨 <b>Rang:</b> {color or 'Standart'}
📞 <b>SIM:</b> {'📲 eSIM' if data.get('sim_type') == 'esim' else '📱 SIM karta'}
...
{'🎁 <b>Bepul urinishdan foydalanildi!</b>' if is_free else '💰 <b>Pullik balansdan foydalanildi!</b>'}
💳 <b>Yangi balans:</b> {balance} ta narxlash
🎁 <b>Bepul urinishlar qoldi:</b> {free_trials_left} ta
...
@dp.message_handler(lambda m: m.text == "🔄 Yana hisoblash", state='*')
