...
def main_menu(is_admin=False):
    ...
    kb.row(KeyboardButton("📱 iPhone narxini bilish"))
    kb.row(KeyboardButton("📊 Statistika"))

    # Agar admin bo'lsa admin tugmalari
    if is_admin:
        kb.row(KeyboardButton("📥 Excel import"), KeyboardButton("👨‍💼 Admin panel"))
...
def admin_kb():
    ...
    # Birinchi qator - 2 ta tugma
    kb.row(KeyboardButton("📊 Statistika"), KeyboardButton("📥 Excel import"))

    # Ikkinchi qator - 2 ta tugma
    kb.row(KeyboardButton("📤 Excel export"), KeyboardButton("🧹 Bazani tozalash"))

    # Uchinchi qator - 2 ta tugma
    kb.row(KeyboardButton("📱 Namuna ma'lumot"), KeyboardButton("◀️ Orqaga"))

    # To'rtinchi qator - 1 ta tugma (markazda)
    kb.row(KeyboardButton("🏠 Bosh menyu"))
    return kb


def parts_choice_kb():
    """Almashgan qism bormi degan savol uchun klaviatura"""
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
...
def sort_models_naturally(models):
    """iPhone modellarini to'g'ri tartibda saralash"""
    ...
    return sorted(models, key=lambda x: extract_number(x['name']), reverse=True)
...
async def back_handler(message: types.Message, state: FSMContext):
    ...
    if current == UserState.waiting_storage.state:
        ...
        await parts_choice_selected(types.Message(text="❌ Yo'q"), state)
...
async def battery_selected(message: types.Message, state: FSMContext):
    ...
    if len(sims) == 1:
        ...
        # Quti bor/yo'q uchun klaviatura
        kb = ReplyKeyboardMarkup(resize_keyboard=True)
...
async def box_selected(message: types.Message, state: FSMContext):
    ...
    # Almashgan qism bormi degan savol
...
async def parts_choice_selected(message: types.Message, state: FSMContext):
    ...
    # Ha bosildi - Inline qismlar tanlovini ko'rsatish
    markup = InlineKeyboardMarkup(row_width=2)
    for key, name in PARTS.items():
        markup.add(InlineKeyboardButton(f"☐ {name}", callback_data=f"part_{key}"))
    markup.add(InlineKeyboardButton("➡️ Davom etish", callback_data="part_done"))

    await message.answer(
        "<b>🔧 Almashgan qismlarni tanlang (maks 3 ta):</b>\n\n"
        "⚠️ <i>Ekran va Oyna birga tanlanmaydi!</i>",
        reply_markup=markup,
        parse_mode="HTML"
    )
...
async def parts_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    for key, name in PARTS.items():
        ...
        markup.add(InlineKeyboardButton(text, callback_data=f"part_{key}"))
    markup.add(InlineKeyboardButton(f"➡️ Davom etish ({len(selected)}/3)", callback_data="part_done"))

    names = ", ".join([PARTS[k] for k in selected]) if selected else "hech qanday"
    await call.message.edit_text(
        f"<b>🔧 Tanlangan qismlar:</b> {names}\n\n"
        f"⚠️ <i>Ekran va Oyna birga tanlanmaydi!</i>",
        reply_markup=markup,
        parse_mode="HTML"
    )
    await call.answer()
...
async def import_start(message: types.Message):
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(KeyboardButton("❌ Bekor qilish"))
    kb.row(KeyboardButton("🏠 Bosh menyu"))
...
# === STATISTIKA (umumiy foydalanuvchilar uchun) ===
...
📱 <b>Modellar:</b> {models_count} ta
💰 <b>Narxlar:</b> {prices_count} ta

<b>📈 Faollik:</b>
- Botda barcha iPhone modellari mavjud
- Har bir model uchun 10+ turdagi narxlar
- Narxlar real bozor asosida

<b>ℹ️ Ma'lumot:</b>
Narxlar taxminiy, bozordagi narxlar farq qilishi mumkin.
"""
